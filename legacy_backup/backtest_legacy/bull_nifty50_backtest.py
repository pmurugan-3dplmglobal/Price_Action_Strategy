...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("output/logs/bull_nifty50_backtest.log", mode="a", encoding="utf-8"),
...
def simulate_trade_outcome(kite, trade, target_date):
    ...
    try:
        ...
        if sl_hit:
            pnl = t1 - entry_premium if t1 else 0
            result = "SL_HIT"
            if exit_ts is None:
...
def run_scan_cycle(kite):
    ...
    for symbol in scan_order:
        ...
        for contract in contracts:
            ...
            # Validate targets in premium space (bull: targets above entry)
...
def main():
    ...
    if LIVE_MARKET_DEPLOYMENT:
        logging.info("Live mode not implemented in backtest engine - use live-trade/bull_nifty50_scanner.py")
    else:
