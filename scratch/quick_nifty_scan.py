...
ref_now = dt.now()
from_entry = (ref_now - timedelta(days=5)).strftime("%Y-%m-%d")
to_entry = ref_now.strftime("%Y-%m-%d")
from_anchor = (ref_now - timedelta(days=15)).strftime("%Y-%m-%d")
...
print("Scanning NIFTY only (ATM +- 1 strike)...")
trades = scan_symbol(
    kite, "NIFTY", INDEX_REGISTRY["NIFTY"], from_entry, to_entry, from_anchor, to_anchor,
    ...
    active_pos, pos_lock, trade_db, 1,
    lambda *args, **kwargs: None
)
print(f"NIFTY trades found: {len(trades)}")
for t in trades:
    print(f"  -> {t['contract']} | {t['pattern']} | Entry: {t['entry_spot']} | SL: {t['current_sl']} | T1: {t['t1']}")
