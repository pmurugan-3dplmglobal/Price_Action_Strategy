...
from resolve import scan_symbol, resolve_option_strikes as shared_resolve_strikes, write_scan_display_data
...
# Recover active
active_trades = trade_db.get_active_trades("index")
for at in active_trades:
    sym = at.get("symbol")
    if sym in INDEX_REGISTRY:
        active_pos[sym] = at
...
for sym, cfg in INDEX_REGISTRY.items():
    ...
    staged_trades.extend(trades)

write_scan_display_data(staged_trades, active_pos, paths.SCAN_DISPLAY_INDEX_FILE, "index")
print(f"Successfully wrote {len(staged_trades)} staged trades and {len(active_pos)} active positions to {paths.SCAN_DISPLAY_INDEX_FILE}")
