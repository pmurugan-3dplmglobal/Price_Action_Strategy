...
from shared.kite_utils import (
    ...
    init_registries, NFO_INSTRUMENTS, is_auth_error, reload_kite_client
)
...
STATE_FILE = "output/monitor/bull_index.pid"
...
def execute_highest_rr_trade(kite, staged):
    ...
    if LIVE_MARKET_DEPLOYMENT:
        try:
            ...
            target_for_rr = best["T1"] if best["T1"] is not None else (best["T2"] if best["T2"] is not None else (best["T3"] if best["T3"] is not None else best["Entry"]))
            rr_best = round((target_for_rr - best["Entry"]) / (best["Entry"] - best["SL"]), 2) if best["SL"] != best["Entry"] else 0
            if rr_best < 1.5:
            ...
            log_to_journal(sym, best["Pattern"], INDEX_TF_ENTRY, "BUY", "SUCCESS",
                           f"Order: {oid}, Qty: {qty}, {side}@{best['Strike']}", entry=best["Entry"], sl=best["SL"], target=target_for_rr, rr=rr_best)
        except Exception as e:
...
def run_anchor_scan(kite):
    """Run anchor scan for all index CE contracts."""
    ...
    for symbol, config in INDEX_REGISTRY.items():
        ...
        step = config["strike_step"]
        contracts = resolve_option_strikes(symbol, spot, step, OPTION_TYPE, INDEX_STRIKE_RANGE, ENGINE_TYPE)
        if not contracts:
            continue
...
def main():
    ...
    if LIVE_MARKET_DEPLOYMENT:
        ...
        logging.info("Use backtest/bull_index_backtest.py for backtesting")
