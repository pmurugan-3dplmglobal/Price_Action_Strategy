...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("output/logs/bear_nifty50_scanner.log", mode="a", encoding="utf-8"),
...
STATE_FILE = "output/monitor/bear_nifty50.pid"
...
def run_anchor_scan(kite):
    """Run anchor scan for all Nifty50 stock PE contracts."""
    ...
    for symbol, config in STOCK_REGISTRY.items():
        ...
        step = config["strike_step"]
        contracts = resolve_option_strikes(symbol, spot, step, OPTION_TYPE, BEAR_NIFTY50_STRIKE_RANGE, ENGINE_TYPE)
        if not contracts:
            continue
...
def main():
    ...
    if LIVE_MARKET_DEPLOYMENT:
        ...
        logging.info("Use backtest/bear_nifty50_backtest.py for backtesting")
